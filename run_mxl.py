...
def main():
    ...
    data_y = T.cast(shared(np.asarray(dataset_y-1, dtype=floatX), borrow=True), 'int32')
    ...
    # symbolic theano tensors
    x_ng = T.tensor3('data_x_ng')
    ...
    # construct model
    model = MixedLogit(sz_i, av, input=[x_ng, x_g], n_in=[(sz_m), (sz_k, sz_i)], draws=draws)
    ...
    fn_estimate = function(
        ...
        updates=updates,
        givens={
            x_ng: data_x_ng[index*sz_minibatch : T.min(((index+1)*sz_minibatch,sz_n))],
            x_g: data_x_g[index*sz_minibatch : T.min(((index+1)*sz_minibatch,sz_n))],
            y: data_y[index*sz_minibatch : T.min(((index+1)*sz_minibatch,sz_n))],
            av: data_av[index*sz_minibatch : T.min(((index+1)*sz_minibatch,sz_n))],
            draws: rng[index*sz_minibatch : T.min(((index+1)*sz_minibatch,sz_n))]},
        allow_input_downcast=True,
        on_unused_input='ignore',)

    """ Main estimation process loop """
    print('Begin estimation...')
    # process loop parameters
    epoch = 0
    sz_epoches = 2500
    ...
    while epoch < sz_epoches and done_looping is False:
        ...
        if this_loglikelihood > best_loglikelihood:
            ...
            with open('best_model.pkl', 'wb') as f: pickle.dump(model, f)
...
def run_analytics(model, hessians):
    ...
    betas = np.concatenate([param.get_value() for param in model.params], axis=0)
    t_stat = betas/stderr
    ...
    ASCs = ['ASC_Bus', 'ASC_CarRental', 'ASC_Car', 'ASC_Plane', 'ASC_TrH', 'ASC_Train']
    nongenericNames =  ['cost', 'tt', 'relib','cost_s', 'tt_s', 'relib_s']
    genericNames = [
         'DrvLicens', 'PblcTrst', \
    #    'Ag1825', 'Ag2545', 'Ag4565', 'Ag65M', 'Male', 'Fulltime', 'PrtTime', 'Unemplyd', 'Edu_Highschl', 'Edu_BSc', 'Edu_MscPhD', 'HH_Veh0', 'HH_Veh1', \
